...
def get_rotary_embedding(hidden_states, position_ids, config):
    ...
    # 调整形状以匹配注意力头的维度
    # 修改：直接返回cos和sin，不改变形状，在apply_rotary_pos_emb中处理
...
def apply_rotary_pos_emb(q, k, cos, sin):
    # 修复维度不匹配问题
    # q和k的形状为[batch_size, num_heads, seq_length, head_dim]
    # cos和sin的形状需要调整为[batch_size, 1, seq_length, head_dim]
...
# # 执行推理获取初始logits
# print(TerminalFormatter.info("执行初始推理..."))
# logits = inference(input_ids, all_parameters, config)
# print(TerminalFormatter.colorize(f"初始logits形状: {logits.shape}", 'bright_cyan'))

# # 比较推理结果
# max_diff, p80_diff = compare_inference_results(input_ids, all_parameters, config)
# 获取最后一个token的logits
# next_token_logits = logits[:, -1, :]

# 获取下一个token ID
# next_token_id = np.argmax(next_token_logits, axis=-1)
# print(TerminalFormatter.colorize(f"第一个预测token ID: {next_token_id}", 'bright_cyan'))

# 解码预测的token
# next_token = tokenizer.decode([next_token_id[0]])
# print(TerminalFormatter.colorize("第一个预测token: ", 'bright_cyan') + 
#       TerminalFormatter.token_display(next_token))

# 将预测的token添加到输入中
# new_input_ids = np.concatenate([input_ids, next_token_id.reshape(1, 1)], axis=1)

# 解码完整输入
# full_text = tokenizer.decode(new_input_ids[0])
# print("\n" + TerminalFormatter.colorize("目前生成的文本:", 'bright_cyan', 'bold'))
# print(TerminalFormatter.colorize(full_text, 'bright_white'))

# print("\n" + TerminalFormatter.warning("要生成完整文本，请运行完整的greedy_decode函数。"))
# print(TerminalFormatter.info("注意：使用全部28层进行完整生成将需要较长时间。"))
